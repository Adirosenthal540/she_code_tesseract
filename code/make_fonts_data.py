...
def convert_wordDocx_to_x_words_in_line_txtFile(x, doc, outputPathForTextFile):
    j=0
    f = open(os.path.join(outputPathForTextFile,  "test_word_file17.txt"), "w+", encoding="utf-8")
...
def extract_ines_of_text_from_images_and_match_labels(folder, txtfile):
    font_name = os.path.basename(folder)[:-8] #remove "_images_"
    if len(font_name)>5:
...
def main():
    ...
    # outputPathForTextFile = r"C:\Users\Adi Rosental\Documents\she_code\shecode_final_project\handwriteDoc\trainFonts"
    # convert_wordDocx_to_x_words_in_line_txtFile(6, doc, outputPathForTextFile)
    #
    ...
    extract_ines_of_text_from_images_and_match_labels(folder, txtfile)
